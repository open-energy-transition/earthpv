"""Fitted panel-pose survey page: polar tilt(radius) x azimuth(angle) plot, tilt/azimuth
histograms, and a detected/validated strip chart, all derived from a glint-validation
summary CSV (`scripts/glint_validate_pakistan.py analyze`'s output shape: one row per
target with `area_m2`, `n_spikes`, `n_consistent`, and, for rows with `n_consistent >= 2`,
`fit_tilt`/`fit_az`/`kind`/`placement`/`pid`).

Originally a one-off script (`scripts/build_pv_pose_country2000.py`) for Pakistan's
country-scale study; pulled out here so a second country's survey can reuse the same
page without copy-pasting the template, as part of the national-dashboard library
(`dashboard.py`).

Only measured points are plotted. An earlier version of this page also drew each
measured point mirrored across the south axis, on the assumption that panel orientation
is symmetric -- that mirrored half was never measured, and duplicating data onto the
side the sensor cannot see manufactures the appearance of coverage where there is none.
Dropped 2026-07-31; the shaded wedge alone communicates "the sensor cannot see this
side" without pretending to have data there. The wedge and its two hard edges are
derived from this run's own observed min/max fitted azimuth, not hardcoded, so a
different country's overpass geometry (a different fixed local crossing time) falls
out correctly without touching this code.
"""
...
<style>
/* Palette: the same "night lights" tokens as the atlas templates and the docs site
   (docs/assets/stylesheets/extra.css). `--accent-sun` is the atlas accent (amber) and
   `--accent-glint` its `--domain` (aqua); the dark scheme used to be a cool navy of its
   own, which read as a foreign panel once this page was embedded in the docs site
   alongside a warm-dark atlas. The serif body face below is this page's own voice and
...
:root {
  ...
  --accent-glint: #1f9b8a;
  --excluded: #e6dcc7;
  --excluded-line: #c3b492;
  --card-shadow: 0 1px 2px rgba(80,55,15,0.07);
  /* Without this the browser paints a light scrollbar over the dark page, which is
     what gives an embedded iframe away. */
  color-scheme: light;
  scrollbar-color: #c3b492 #ece4d2;
}
@media (prefers-color-scheme: dark) {
  :root {
    --bg: #100d09; --bg-panel: #1a160f; --bg-chart: #221d14;
    --ink: #f7f1e6; --ink-dim: #c9bda4; --ink-faint: #93866c;
    --rule: #3a2d16; --accent-sun: #f5a623; --accent-glint: #2fd9c4;
    --excluded: #241f16; --excluded-line: #4a3a1c; --card-shadow: none;
    color-scheme: dark;
...
:root[data-theme="light"] {
  --bg: #f1ebdd; --bg-panel: #faf6ec; --bg-chart: #f3ecdb;
  --ink: #2a2216; --ink-dim: #5f5540; --ink-faint: #857a61;
  --rule: #cfc0a5; --accent-sun: #c25e12; --accent-glint: #1f9b8a;
  --excluded: #e6dcc7; --excluded-line: #c3b492; --card-shadow: 0 1px 2px rgba(80,55,15,0.07);
  color-scheme: light;
  scrollbar-color: #c3b492 #ece4d2;
}
...
<div class="wrap">
  <header>
    <p class="eyebrow mono">__EYEBROW__</p>
    <h1>__H1__</h1>
    <p class="dek">
      ...
      two years of imagery__DEK_HISTORY__.
  ...
  <div class="grid">
    <div class="panel chart-panel">
      <div class="chart-head">
        ...
        <span class="chart-n mono">__N_FITTED__ measured</span>
      </div>
      <div id="polar-wrap">
      ...
      <div class="legend">
        ...
        <span class="legend-item"><span class="swatch excl"></span> unreachable by this sensor's fixed overpass time</span>
      </div>
    </div>
    ...
    <div class="sidebar">
      ...
      <div class="panel callout">
        <h3>Why the plot only fills part of the circle</h3>
        <p>
          ...
          rooftops in __COUNTRY__: the <span class="fig">shaded wedge</span>
          (<span class="fig">__WEDGE_DEG__&deg;</span> wide) marks orientations this survey
          cannot observe at all, not orientations known to be empty. Nothing is plotted there,
  ...
  <footer>
    <p>
      Data: <span class="mono">__SUMMARY_PATH__</span>__DATA_NOTE__. Validated = a single
...
def compute_pose_survey_data(
    ...
    # Everything outside the observed [az_min, az_max] range is unreachable by this
    # sensor's fixed overpass geometry (see module docstring) -- shaded as one wedge
    # rather than assumed-filled via a mirror.
    wedge = [round(az_max, 1), round(az_min + 360.0, 1)]
    wedge_span = round(wedge[1] - wedge[0], 1)

    points = [
        dict(p=r.pid, k=r.kind, pl=r.placement, a=round(float(r.area_m2), 1),
             t=round(float(r.fit_tilt), 1), az=round(float(r.fit_az), 1),
             n=int(r.n_consistent))
